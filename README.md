...
> NAME_OF_THE_FOLDER_WITH_THE_DATA (put as data path)
  > raw_ref_video_hdr
        - contains the whole HDR images, so the test HDR images
        - hdr format
        - contains both ref and video images
  > raw_video_png
        - contains the whole LDR images, so the test LDR images
        - png format
        - contains only video images
  > tile_ref_video_hdr
        - contains the tiled HDR images, so the train HDR images
        - hdr format
        - contains both ref and video images
  > tile_ref_video_png
        - contains the tiled LDR images, so the train LDR images
        - png format
        - contains both ref and video images
```
